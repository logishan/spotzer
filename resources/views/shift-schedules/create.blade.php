...
@section('content')
    <div class="page-header">
        <h1 class="page-title">Create Shift Schedule</h1>
        <div>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('shift-schedules.index') }}">Shift Schedules</a></li>
                <li class="breadcrumb-item active" aria-current="page">Create Shift Schedule</li>
            </ol>
        </div>
    </div>
        ...
        <div class="col-md-12 col-xl-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Create New Shift Schedule</h3>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('shift-schedules.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control select2 @error('days_of_week') is-invalid @enderror" id="days_of_week" name="days_of_week[]" multiple>
                                <option value="0" {{ (old('days_of_week') && in_array('0', old('days_of_week'))) ? 'selected' : '' }}>Sunday</option>
                                <option value="1" {{ (old('days_of_week') && in_array('1', old('days_of_week'))) ? 'selected' : '' }}>Monday</option>
                                <option value="2" {{ (old('days_of_week') && in_array('2', old('days_of_week'))) ? 'selected' : '' }}>Tuesday</option>
                                <option value="3" {{ (old('days_of_week') && in_array('3', old('days_of_week'))) ? 'selected' : '' }}>Wednesday</option>
                                <option value="4" {{ (old('days_of_week') && in_array('4', old('days_of_week'))) ? 'selected' : '' }}>Thursday</option>
                                <option value="5" {{ (old('days_of_week') && in_array('5', old('days_of_week'))) ? 'selected' : '' }}>Friday</option>
                                <option value="6" {{ (old('days_of_week') && in_array('6', old('days_of_week'))) ? 'selected' : '' }}>Saturday</option>
                            </select>
                            @error('days_of_week')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
